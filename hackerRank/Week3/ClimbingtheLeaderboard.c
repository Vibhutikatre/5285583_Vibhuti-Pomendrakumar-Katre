 ...
 *
 */
int* climbingLeaderboard(int total_count, int* scores, int total_players, int* players, int* fail_count) {
    int* ranking = malloc(total_players * sizeof(int));
    *fail_count = total_players;

    int* L_board = malloc(total_count * sizeof(int));
    int board_len = 0;
    L_board[board_len++] = scores[0];
    for (int i = 1; i < total_count; i++) {
        if (scores[i] != scores[i - 1]) {
            L_board[board_len++] = scores[i];
        }
    }

    int idx = board_len - 1;
    for (int j = 0; j < total_players; j++) {
        while (idx >= 0 && players[j] >= L_board[idx]) {
            idx--;
        }
        ranking[j] = idx + 2;  
    }
