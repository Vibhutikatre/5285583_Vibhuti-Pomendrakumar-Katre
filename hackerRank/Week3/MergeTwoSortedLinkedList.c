 ...
 *
 */
SinglyLinkedListNode* mergeLists(SinglyLinkedListNode* first_H, SinglyLinkedListNode* second_H) {
    
    if (!first_H) return second_H;
    if (!second_H) return first_H;

    if (first_H->data <= second_H->data) {
        first_H->next = mergeLists(first_H->next, second_H);
        return first_H;
    } else {
        second_H->next = mergeLists(first_H, second_H->next);
        return second_H;
    } 
}                                                                                
