...
int** seq = calloc(n, sizeof(int*));
    int* length = calloc(n, sizeof(int));
    int* solution = malloc(queries_rows * sizeof(int));
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (t == 1) { 
            seq[idx] = realloc(seq[idx], (length[idx] + 1) * sizeof(int));
            seq[idx][length[idx]++] = y;
        } else {
            lastAnswer = seq[idx][y % length[idx]];
            solution[count++] = lastAnswer;
        }
    }
    *result_count = count;
    return solution;
